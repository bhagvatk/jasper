...
public class JasperHelper {
	...
	static {
		...
		gridColumnStyle = stl.style(JasperHelper.columnStyle).setBorder(
				stl.pen1Point());
		...
		StyleBuilder crosstabCellStyle = stl.style(columnStyle)
				.setBorder(stl.pen1Point()).setBackgroundColor(Color.WHITE)
				.setPadding(2);
		...
		dynamicReportsComponent = cmp
				.horizontalList(
						cmp.image("camera.png").setFixedDimension(60,
								60),
						cmp.verticalList(cmp
								.text("ExtremityIndia Technologies")
								.setStyle(bold12CenteredStyle)
								.setHorizontalAlignment(
										HorizontalAlignment.CENTER)));

		footerComponent = cmp.horizontalList(
				cmp.text("Report Generated on" + " "
						+ DateFormat.getDateTimeInstance().format(new Date())),
				cmp.pageXofY()).setStyle(
				stl.style().setTopBorder(stl.pen1Point()));
	}
	...
	public static ComponentBuilder<?, ?> createTitleComponent(String label) {
		...
		return cmp
				...
				.newRow(0)
				.add(cmp.text("as on" + " " + DateToStr).setStyle(
						boldCenteredStyle)).newRow(0).add(cmp.line()).newRow(0)
